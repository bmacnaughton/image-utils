...
// writes script commands
console.log("#!/bin/bash");
console.log("set -e");

const allDirs = await fs.readdir('/mnt/z/xiaoxin-bruce/pictures');
...
for (const spacedDir of spacedDirs) {
  ...
  if (allDirs.includes(dashedDir)) {
    ...
    await fs.rename(spacedDir, dashedDir);
  }

  const spacedFiles = await fs.readdir(spacedDir);
  const dashedFiles = await fs.readdir(dashedDir);
