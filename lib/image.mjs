...
export default class Image {
  constructor(file, options = {}) {
    ...
    if (options.buf && !Buffer.isBuffer(options.buf)) {
      throw new Error('options.buf must be a Buffer');
    }

    const initialPromise = options.buf ? Promise.resolve(options.buf) : fsp.readFile(file);
    this.readPromise = initialPromise
      .then(buf => {
        this.buf = buf;
